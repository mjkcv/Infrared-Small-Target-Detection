...
class region_propose(nn.Module):
    ...
    def forward(self, x):
        _, _, h, w = x.shape
        detect_output, boxes = self.backbone(x, device='cuda')
...
if __name__ == '__main__':
    ...
    backbone = backbone()
    print(backbone)
    # output1, output2 = backbone(input)
    # print(output1.shape, output2.shape)
    # model = region_propose(backbone)
    # output = model(input)
    # output = backbone.detect(input)
    #
    #
    # print(output.shape)
    # input = torch.rand(1,6,146,256)
