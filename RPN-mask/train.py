...
if __name__ == '__main__':
    ...
    parser.add_argument("--bboxfile", type=str, default="./cGAN_data/training_box_gt.csv", help="path to bounding boxes ground truth")
    ...
    parser.add_argument('--rpn_pretrained', type=str, default='', help='load pretrained rpn weights')
    ...
    if args.rpn_pretrained:
        Module = torch.load(args.rpn_pretrained)
    ...
    lr_scheduler = torch.optim.lr_scheduler.LambdaLR(optimizer, lr_lambda=fun_1, verbose=True)
    ...
    for epoch in range(args.epochs):
        ...
        mloss = np.zeros(3, dtype=np.float)
        for i, (input, seg_targets, bbox_targets) in pbar:

            detect_output, _ = Model(input.to(device))
            loss, loss_items = Dcriterion(p=detect_output, target=bbox_targets.float().to(device),
        ...
        # Loss_list.append(mloss[2])
        # print(f'{reg_output.sigmoid().max()}')
        #
        print(('%10s' * 3) % ('box', 'obj', 'Dloss'))
        print(('%10.4g' * 3) % (mloss[0], mloss[1], mloss[2]))
